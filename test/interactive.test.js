...
const body = (kind = 'proposal', extra = {}) => ({ clientMessageId: randomUUID(), kind, summary: 'Strict comparison respects half-open boundaries.', evidence: ['Adjacent intervals do not overlap.'], ...extra });
...
test('pause blocks new work and replacement, clears leases, and preserves independent sealing', async t => {
  const { root, collab } = await fixture(t, { checks: { unit: [process.execPath, '-e', 'process.exit(0)'] } });
...
test('resume restores remaining deadline time even after the original deadline passes', async t => {
  ...
  assert.ok(remaining > 4500 && remaining <= 5000, `Expected frozen remaining budget, got ${remaining} ms`);
...
test('human notes invalidate votes and checks and require explicit current-context acknowledgement', async t => {
  ...
  await assert.rejects(collab.post('claude', body('evidence', { contextVersion: 0 })), /Shared context changed/);
  await assert.rejects(collab.post('claude', body('accept', { contextVersion: 1, candidate: first.message.id })), /Required checks/);
  await collab.post('codex', body('evidence', { contextVersion: 1, summary: 'Reviewed the new empty-interval requirement.' }));
  await collab.post('claude', body('evidence', { contextVersion: 1, summary: 'Added an empty interval counterexample.' }));
  const after = await collab.status();
  assert.equal(after.session.round, 2);
  assert.deepEqual(after.session.messages.slice(-2).map(message => message.contextVersion), [1, 1]);
  assert.match(await fs.readFile(path.join(root, '.collab', 'README.md'), 'utf8'), /User note · context 1/);
});

test('a human note invalidates a verification already running', { timeout: 5000 }, async t => {
  const script = 'const fs=require("node:fs");fs.writeFileSync("check.started","yes");const timer=setInterval(()=>{if(fs.existsSync("check.finish")){clearInterval(timer);process.exit(0)}},10);';
  const { root, collab } = await fixture(t, { checks: { unit: [process.execPath, '-e', script] } });
  await fs.writeFile(path.join(root, 'answer.js'), 'export default 42;');
  const first = await collab.post('codex', body('proposal', { files: ['answer.js'] }));
  const checking = assert.rejects(collab.verify('codex', first.message.id, 'unit'), /Shared context changed/);
  let started = false;
  try {
    for (let attempt = 0; attempt < 100; attempt++) {
      try { await fs.access(path.join(root, 'check.started')); started = true; break; }
  ...
  } finally { await fs.writeFile(path.join(root, 'check.finish'), 'yes'); }
  await checking;
  assert.deepEqual((await collab.status()).session.checks, []);
});

test('awaitTurn ignores claim and verification revisions until the peer advances the round', { timeout: 5000 }, async t => {
  const { root, collab } = await fixture(t, { checks: { unit: [process.execPath, '-e', 'process.exit(0)'] } });
  await fs.writeFile(path.join(root, 'answer.js'), 'export default 42;');
  const first = await collab.post('codex', body('proposal', { files: ['answer.js'] }));
  let settled = false;
  const waiting = collab.awaitTurn('codex', 2000).then(state => { settled = true; return state; });
...
test('awaitTurn returns for pause, terminal state, new human context, timeout and cancellation', { timeout: 5000 }, async t => {
  for (const action of ['pause', 'stop', 'note', 'timeout', 'abort']) {
    const { collab } = await fixture(t);
    await collab.post('codex', body());
    const controller = new AbortController();
    const waiting = collab.awaitTurn('codex', action === 'timeout' ? 10 : 2000, controller.signal);
    if (action === 'pause') await collab.pause();
    if (action === 'stop') await collab.stop('Human ended the goal.');
    if (action === 'note') await collab.note('New requirement makes another contribution actionable.');
    if (action === 'abort') controller.abort();
    const result = await waiting;
    if (action === 'pause' || action === 'stop') assert.equal(result.session.status, action === 'pause' ? 'paused' : 'stopped');
    else if (action === 'note') {
