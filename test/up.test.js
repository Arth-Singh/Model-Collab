...
async function fixture(t, unusual = false) {
  ...
  const root = unusual
    ? path.join(base, `RL 'single' "double" $(touch PATH_INJECTION) \`touch PATH_TICK\``)
...
function fakeTmux() {
  ...
  fake.run = async (argv) => {
    ...
    if (fake.fail === command)
      return { code: 1, stdout: '', stderr: `Injected ${command} failure` };
    if (command === 'has-session')
      return { code: sessions.has(option('-t')) ? 0 : 1, stdout: '', stderr: '' };
    ...
    if (command === 'show-options')
      return { code: 0, stdout: sessions.get(option('-t')).owner + '\n', stderr: '' };
  ...
  async function visit(directory) {
    for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await visit(absolute);
        ...
        files[path.relative(root, absolute)] = await fs.readFile(absolute, 'utf8');
    }
  }
  await visit(root);
  return files;
...
test('print plans arbitrary goals without filesystem or subprocess side effects, including expired sessions', async (t) => {
  ...
  assert.equal(plan.config.preset, 'coding');
...
test('startup preserves project instructions and context and passes unusual paths as literal argv', async (t) => {
  ...
  const preserved = {
    'AGENTS.md': 'Existing repository agent contract\n',
  ...
  const fake = fakeTmux();
  const result = await up({ root, goal: dangerous, preset: 'research', runTmux: fake.run, models });
  ...
  for (const name of ['START-CODEX.md', 'START-CLAUDE.md'])
    assert.ok((await fs.readFile(path.join(root, '.collab', name), 'utf8')).length > 100);
  ...
  for (const argv of nativeCalls) {
    ...
    assert.ok(
      !argv.includes(dangerous),
      'Goal belongs in state, never interpolated into a shell command',
    );
  }
});
...
test('a tmux ownership mismatch is rejected without killing or changing existing resources', async (t) => {
  ...
  assert.deepEqual(
    fake.calls.slice(beforeCalls).map((argv) => argv[2]),
    ['has-session', 'show-options'],
  );
});

test('attach checks current ownership and constructs its own argv instead of trusting saved commands', async (t) => {
  const { root } = await fixture(t, true);
  const fake = fakeTmux();
  const started = await up({ root, goal: 'Inspect the current project.', runTmux: fake.run });
  const file = path.join(root, '.collab', 'terminal.json');
  const saved = JSON.parse(await fs.readFile(file, 'utf8'));
  await fs.writeFile(
    file,
    JSON.stringify({ ...saved, attach: ['untrusted-command'], attachCommand: 'untrusted-command' }),
  );
  const run = async (argv) => {
    assert.equal(argv[0], 'tmux');
    return fake.run(argv.slice(1));
  };
  const target = await findTerminal({ root, run });
  assert.deepEqual(target.attach, [
    'tmux',
    '-L',
    'model-collab',
    'attach-session',
  ...
  fake.sessions.get(started.sessionName).owner = 'different project';
  await assert.rejects(findTerminal({ root, run }), /not owned/);
  fake.sessions.delete(started.sessionName);
  await assert.rejects(findTerminal({ root, run }), /session has closed/);
  const collab = new Collaboration(root);
  await collab.stop();
  await collab.start({ topic: 'Replacement goal.' });
  await assert.rejects(findTerminal({ root, run }), /earlier goal/);
});

test(
  'workers fallback starts both peers with injected turns and no paid provider calls',
  { timeout: 5000 },
  async (t) => {
    const { root } = await fixture(t);
    const turns = [];
      ...
      turn: async ({ agent, state }) => {
        ...
        if (state.session.phase === 'independent')
          return {
            ...
            evidence: ['Touching endpoints do not overlap.'],
...
test(
  ...
  async (t) => {
    ...
    const code =
      'require("node:fs").writeFileSync(process.argv[1],JSON.stringify({payload:process.argv[2],cwd:process.cwd()}));setInterval(()=>{},1000);';
    let ownedName;
    const run = async (argv) => {
    ...
    t.after(async () => {
      if (ownedName)
        await runCommand(['tmux', '-L', 'model-collab', 'kill-session', '-t', ownedName], {
          timeoutMs: 3000,
        });
    });
    const paneCommands = Object.fromEntries(
      ['codex', 'claude'].map((agent) => [
        agent,
        [process.execPath, '-e', code, path.join(root, `${agent}.json`), payload],
      ]),
    );
    const result = await startTmux({
      root,
