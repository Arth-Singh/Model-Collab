...
export const peerContract = await fs.readFile(
  new URL('../prompts/peer.md', import.meta.url),
  'utf8',
);
const interactiveTemplate = await fs.readFile(
  new URL('../prompts/interactive.md', import.meta.url),
  'utf8',
);
export const researchContract = await fs.readFile(
...
export async function writeInstructions(root, participants, { preset = 'coding' } = {}) {
  ...
  for (const agent of participants) {
    await fs.writeFile(
      path.join(dir, 'prompts', `${agent}.md`),
      `Your participant ID is ${agent}.\n\n${contract}`,
    );
    const replacements = {
      '%AGENT%': agent,
      '%REPO%': path.resolve(root),
      '%QUOTED_REPO%': shellQuote(path.resolve(root)),
      '%CLI%': `${shellQuote(process.execPath)} ${shellQuote(binPath)}`,
      '%PEER_CONTRACT%': contract,
    };
    const text = interactiveTemplate.replace(
      /%AGENT%|%REPO%|%QUOTED_REPO%|%CLI%|%PEER_CONTRACT%/g,
  ...
  const ignore = await fs.readFile(ignorePath, 'utf8').catch((e) => {
