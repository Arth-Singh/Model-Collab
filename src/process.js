...
export function runCommand(argv, { cwd, timeoutMs = 60000, maxBytes = 128000, input, env } = {}) {
  return new Promise(resolve => {
    ...
    const child = spawn(argv[0], argv.slice(1), { cwd, env, shell: false, detached: process.platform !== 'win32', stdio: ['pipe', 'pipe', 'pipe'] });
    let stdout = '', stderr = '', bytes = 0, reason = null, done = false;
    const kill = () => {
    ...
    function collect(key, chunk) {
      bytes += chunk.length;
      if (bytes > maxBytes) { reason = 'output_limit'; kill(); return; }
